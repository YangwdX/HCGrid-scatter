...
void init_input_with_cpu(const int &sort_param, uint32_t dim) {
    ...
    for(int i=0; i < data_shape; ++i){
        tempArray[i] = hh_data[dim][V[i].inx];
    }
    swap(hh_data[dim], tempArray);
    DEALLOC(tempArray);
...
void data_h2d(uint32_t stream_index, uint32_t dim_index){
    uint32_t data_shape = h_GMaps.data_shape;
    uint32_t num = h_zyx[0] * h_zyx[1] * h_zyx[2];
    uint32_t usedrings = h_Healpix.usedrings;

    // Copy constants memory
    HANDLE_ERROR(cudaMemcpyAsync(d_lons, h_lons, sizeof(double)*data_shape, cudaMemcpyHostToDevice,stream[stream_index]));
    HANDLE_ERROR(cudaMemcpyAsync(d_lats, h_lats, sizeof(double)*data_shape, cudaMemcpyHostToDevice,stream[stream_index]));
    HANDLE_ERROR(cudaMemcpyAsync(d_data, hh_data[dim_index], sizeof(double)*data_shape, cudaMemcpyHostToDevice,stream[stream_index]));
    HANDLE_ERROR(cudaMemcpyAsync(d_weights, h_weights, sizeof(double)*data_shape, cudaMemcpyHostToDevice,stream[stream_index]));
    HANDLE_ERROR(cudaMemcpyAsync(d_xwcs, h_xwcs, sizeof(double)*num, cudaMemcpyHostToDevice,stream[stream_index]));
    HANDLE_ERROR(cudaMemcpyAsync(d_ywcs,h_ywcs, sizeof(double)*num, cudaMemcpyHostToDevice,stream[stream_index]));
    HANDLE_ERROR(cudaMemcpyAsync(d_datacube, h_datacube, sizeof(double)*num, cudaMemcpyHostToDevice,stream[stream_index]));
    HANDLE_ERROR(cudaMemcpyAsync(d_weightscube, h_weightscube, sizeof(double)*num, cudaMemcpyHostToDevice,stream[stream_index]));
    HANDLE_ERROR(cudaMemcpyAsync(d_hpx_idx, h_hpx_idx, sizeof(uint64_t)*(data_shape+1), cudaMemcpyHostToDevice,stream[stream_index]));
...
void data_d2h(uint32_t index){
    uint32_t num = h_zyx[0] * h_zyx[1] * h_zyx[2];
    HANDLE_ERROR(cudaMemcpyAsync(h_datacube, d_datacube, sizeof(double)*num, cudaMemcpyDeviceToHost,stream[index]));
    HANDLE_ERROR(cudaMemcpyAsync(h_weightscube, d_weightscube, sizeof(double)*num, cudaMemcpyDeviceToHost,stream[index]));
}
...
void solve_gridding(const char *infile, const char *tarfile, const char *outfile, const char *sortfile, const int& param, const int &bDim) {
    ...
    for(int i=0; i<h_GMaps.spec_dim;i++){
        ...
        //     init_input_with_thrust(param);
        // } else {
        //     init_input_with_cpu(param,i);
        // } 
        init_output();
        // HANDLE_ERROR(cudaMemcpy((void**)d_start_ring, h_start_ring, sizeof(uint32_t)*(h_Healpix.usedrings+1), cudaMemcpyHostToDevice));
        HANDLE_ERROR(cudaMalloc((void**)& d_start_ring, sizeof(uint32_t)*(h_Healpix.usedrings+1)));
        cudaError_t error = cudaGetLastError();
        HANDLE_ERROR(cudaBindTexture(NULL, tex_start_ring, d_start_ring, sizeof(uint32_t)*(h_Healpix.usedrings+1)));
